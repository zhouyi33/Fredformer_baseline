...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        self.model = Fredformer_backbone(ablation=ablation, mlp_drop=mlp_drop, use_nys=use_nys, output=output,
                                         mlp_hidden=mlp_hidden, c_in=c_in, context_window=context_window,
                                         ...
                                         subtract_last=subtract_last, verbose=verbose, cf_dim=cf_dim, cf_depth=cf_depth,
                                         cf_heads=cf_heads, cf_mlp=cf_mlp, cf_head_dim=cf_head_dim, cf_drop=cf_drop,
                                         **kwargs)
